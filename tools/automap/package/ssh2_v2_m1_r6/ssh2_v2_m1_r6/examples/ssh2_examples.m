...
% ssh_conn - This struct should be reused and savszed with all
% of these functions. For more information, please see the non-simple
% examples below.

HOSTNAME = 'portoalegre.andrew.cmu.edu';
USERNAME = 'nilm-data';
PASSWORD = 'nilm';
...
files = ssh2_simple_command(HOSTNAME,USERNAME,PASSWORD,' ls /media/nilm-data/Inferlab/Respawn/final/');
...
for i=1:10
    a{i} = ssh2_simple_command(HOSTNAME,USERNAME,PASSWORD,['cat /media/nilm-data/Inferlab/Respawn/final/',files{1}]);
end
...
ssh2_conn = scp_simple_get(HOSTNAME,USERNAME,PASSWORD,'jingkun.json','.','/media/nilm-data/Inferlab/Respawn');
