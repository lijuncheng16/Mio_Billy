...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"Usage: %s <-event event_node> <-id transducer_reg_id> <-value transducer_typed_value> <-rawvalue transducer_raw_value> <-u username> <-p password> [-verbose]\n",
			prog_name);
...
int main(int argc, char **argv) {
	...
	if (username == NULL ) {
		...
		fprintf(stderr, "Raw Value missing\n");
		print_usage(argv[0]);
		return -1;
	...
	mio_item_transducer_value_add(item, NULL, t_id, value, rawvalue,
		time_str);
