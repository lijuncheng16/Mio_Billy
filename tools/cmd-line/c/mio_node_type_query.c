...
void print_usage(char *prog_name) {
	fprintf(stdout, "\"%s\" Command line utility to query the type of a node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-node node> <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout, "\t-node node = name of node to query\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	while (current_arg_num < argc) {
		...
		if (strcmp(current_arg_name, "-node") == 0) {
			node = current_arg_val;
	...
	err = mio_node_type_query(conn, node, response);
	if (err == MIO_OK)
