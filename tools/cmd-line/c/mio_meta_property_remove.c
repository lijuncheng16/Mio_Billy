...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"Usage: %s <-event event_node> <-name property_name> <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout, "\t-event event_node = name of event node to remove property from\n");
	fprintf(stdout, "\t-name property_name = name of property to remove\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	mio_meta_remove_publish(conn, event_node, NULL, NULL, 0, &name, 1, response);
