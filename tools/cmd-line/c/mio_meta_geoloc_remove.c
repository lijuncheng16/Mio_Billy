...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"Usage: %s <-event event_node> [-t_name transducer_name] <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout, "\t-event event_node = name of event node to remove geoloc from\n");
	fprintf(stdout, "\t-name transducer_name = name of transducer to remove geoloc from (if not specified, geoloc will be removed from device registered to event node)\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	mio_meta_geoloc_remove_publish(conn, event_node, name, response);
