 ...
 *  Unsubscribe from Event Node Tool
...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"\"%s\" Command line utility to remove a JID as a subscriber of an event node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-event event_node> <-u username> <-p password> [-subid] [-verbose]\n",
			prog_name);
	...
	fprintf(stdout, "\t-subid = subid of the node to unsubscribe\n");
...
int main(int argc, char **argv) {
	...
	while (current_arg_num < argc) {
		...
		} else if (strcmp(current_arg_name, "-subid") == 0) {
			subid = current_arg_val;
	...
	err = mio_unsubscribe(conn, event_node, subid, response);
