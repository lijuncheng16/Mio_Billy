...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"Usage: %s <-event event_node> <-name property_name> <-value property_value> [-t_name transducer_name] <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout, "\t-event event_node = name of event node to publish to\n");
	fprintf(stdout, "\t-name property_name = name of property to add\n");
	fprintf(stdout, "\t-value property_value = value of property to add\n");
	fprintf(stdout,
			"\t-t_name transducer_name = Transducer name to which to add the property to. Property will be added to meta stanza if no transducer name is supplied.\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	mio_property_meta_t *property = mio_property_meta_new();
	mio_transducer_meta_t *t = NULL;
	...
	while (current_arg_num < argc) {
		...
		if (strcmp(current_arg_name, "-event") == 0) {
			...
			property->name = malloc(
					sizeof(char) * (strlen(current_arg_val) + 1));
			strcpy(property->name, current_arg_val);
		} else if (strcmp(current_arg_name, "-value") == 0) {
			property->value = malloc(
					sizeof(char) * (strlen(current_arg_val) + 1));
			strcpy(property->value, current_arg_val);
		} else if (strcmp(current_arg_name, "-t_name") == 0) {
			t = mio_transducer_meta_new();
			t->name = strdup(current_arg_val);
	...
	if (username == NULL ) {
		...
		fprintf(stderr, "Property name missing\n");
		print_usage(argv[0]);
		return -1;
	} else if (property->value == NULL ) {
		fprintf(stderr, "Property value missing\n");
		print_usage(argv[0]);
		return -1;
	...
	if(t == NULL){
		mio_meta_merge_publish(conn, event_node, NULL, NULL, property, response);
		mio_property_meta_free(property);
	}
	else{
		t->properties = property;
		mio_meta_merge_publish(conn, event_node, NULL, t, NULL, response);
		mio_transducer_meta_free(t);
	}
