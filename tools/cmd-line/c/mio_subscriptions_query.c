 ...
 *  Query Event Node Subscribers Tool
...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"\"%s\" Command line utility to list all current subscribers of a JID or event node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-u username> <-p password> [-event event_node] [-verbose]\n",
			prog_name);
...
int main(int argc, char **argv) {
	...
	err = mio_subscriptions_query(conn, event_node, response);
