 ...
 *  Add Publisher to Event Node Tool
...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"\"%s\" Command line utility to add a JID as a publisher of an event node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-event event_node> <-publisher publisher_JID> <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout,
			"\t-publisher publisher_JID = Full JID of publisher to add\n");
	fprintf(stdout, "\t-event event_node = name of node to add publisher to\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	while (current_arg_num < argc) {
		...
		if (strcmp(current_arg_name, "-publisher") == 0) {
			pub_name = current_arg_val;
	...
	if (username == NULL ) {
		...
		print_usage(argv[0]);
		return -1;
	} else if (pub_name == NULL ) {
		fprintf(stderr, "Publisher JID missing\n");
		print_usage(argv[0]);
		return -1;
	...
	if (verbose) {
		...
		fprintf(stdout, "Publisher JID: %s\n", pub_name);
	...
	err = mio_acl_affiliation_set(conn, event_node, pub_name,
			MIO_AFFILIATION_PUBLISHER, response);
