...
void print_usage(char *prog_name) {
	fprintf(stdout, "\"%s\" Command line utility to create a collection node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-collection collection_node> <-u username> <-p password> [-name collection_node_name] [-verbose]\n",
			prog_name);
...
int main(int argc, char **argv) {
	...
	if (verbose) {
		...
		fprintf(stdout, "Collection Node: %s\n", collection_node);
	...
	err = mio_collection_node_create(conn, collection_node, name, response);
