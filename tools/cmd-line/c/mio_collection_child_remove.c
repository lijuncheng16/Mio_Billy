...
mio_stanza_t *collection_stanza = NULL;

void print_usage(char *prog_name) {
	fprintf(stdout,
			"\"%s\" Command line utility to remove a child node from a collection node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-parent collection_node> <-child event_or_collection_node> <-u username> <-p password> [-verbose]\n",
			prog_name);
...
int main(int argc, char **argv) {
	...
	while (current_arg_num < argc) {
		...
		if (strcmp(current_arg_name, "-child") == 0) {
	...
	err = mio_collection_child_remove(conn, child, parent, response);
