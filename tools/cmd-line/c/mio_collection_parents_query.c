...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"\"%s\" Command line utility to retrieve the parents of a collection or event node\n",
			prog_name);
	fprintf(stdout,
			"Usage: %s <-node collection_or_event_node> <-u username> <-p password> [-verbose]\n",
			prog_name);
...
int main(int argc, char **argv) {
	...
	err = mio_collection_parents_query(conn, node, response);
