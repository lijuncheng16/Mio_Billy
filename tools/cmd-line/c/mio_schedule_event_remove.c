...
void print_usage(char *prog_name) {
	fprintf(stdout,
			"Usage: %s <-event event_node> <-id event_id> <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
	fprintf(stdout, "\t-event event_node = name of event node to remove property from\n");
	fprintf(stdout, "\t-name transducer_name = name of transducer to remove\n");
	fprintf(stdout,
...
int main(int argc, char **argv) {
	...
	if (username == NULL ) {
		...
		fprintf(stderr, "Event ID invalid or mission\n");
		print_usage(argv[0]);
		return -1;
	...
	mio_schedule_event_remove_publish(conn, event_node, id, response);
