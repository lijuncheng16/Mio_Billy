...
void print_usage(char *prog_name) {
	fprintf(stdout, "\"%s\" Command line utility to automap a device given the uuid\n",
			prog_name);
	fprintf(stdout, "Usage: %s <-event event_node> <-u username> <-p password> [-verbose]\n",
			prog_name);
	fprintf(stdout, "Usage: %s -help\n", prog_name);
    fprintf(stdout, "\t-event event_node = name of event node to automap to\n");
	fprintf(stdout,
...
void print_meta(){
    ...
    fprintf(stdout, "<device uuid=\"ca2c5086873d547c1e93587be70bbe26\">\n\
   	<transducer>Light Meter</transducer>\n\
   	<transducer>Humidity Sensor</transducer>\n\
  	<transducer>Motion Sensor</transducer>\n\
</device>\n");
}
