...
class MultiDeviceApplication(ApplicationServiceElement, Logging):

    def __init__(self, localDevices, localAddress, aseID=None):
        if _debug: MultiDeviceApplication._debug("__init__ %r %r aseID=%r", localDevices, localAddress, aseID)
        ApplicationServiceElement.__init__(self, aseID)
        ...
        if isinstance(localAddress, Address):
            ...
            self.localAddress = Address(localAddress)
        # map from obj_id to object
        self.objectIdentifiers = {}
        # map from object_name to object
        self.devices = {}

        # local objects by ID and name
        for localDeviceID in localDevices.keys():
            localDevice = localDevices[localDeviceID]
            self.devices[localDeviceID]={}
            self.devices[localDeviceID][localDevice.objectName]=localDevice
            self.objectIdentifiers[localDeviceID] = localDevice

    def add_object(self, dev_id, obj):
        """Add an object to the local collection."""
        if _debug:
            MultiDeviceApplication._debug("add_object %r", obj)
        ...
        # make sure it hasn't already been defined
        if object_name in list(self.devices[dev_id].keys()):
            raise RuntimeError, "already an object with name '%s'" % (object_name,)
        if object_identifier in self.objectIdentifiers:
            raise RuntimeError, "already an object with identifier %s" % (object_identifier,)

        # now put it in local dictionaries
        self.devices[dev_id][object_name] = obj
        self.objectIdentifiers[object_identifier[1]] = obj

        # append the new object's identifier to the device's object list
        self.localDevices[dev_id].objectList.append(object_identifier)

    def delete_object(self, dev_id, obj):
        """Add an object to the local collection."""
        if _debug:
            MultiDeviceApplication._debug("delete_object %r", obj)
        dev = self.devices[dev_id]
        # extract the object name and identifier
        object_name = obj.object_name
        object_identifier = obj.objectIdentifier
        ...
        # delete it from the application
        del dev.objectNames[object_name]
        del objectIdentifiers[object_identifier]

        # remove the object's identifier from the device's object list
        indx = self.localDevices[dev_id].objectList.index(object_identifier)
        del self.localDevices[dev_id].objectList[indx]

    def get_object_id(self, objid):
        """Return a local object or None."""
        return self.objectIdentifiers.get(objid, None)

    def get_object_name(self, dev_id,objname):
        """Return a local object or None."""
        dev = self.devices[dev_id]
        return dev.objectNames.get(objname, None)

    def iter_objects(self,dev_id):
        """Iterate over the objects."""
        dev = self.devices[dev_id]
        return dev.objectIdentifiers.itervalues()
    ...
    def indication(self, apdu):
        ...
        helperName = "do_" + apdu.__class__.__name__
        helperFn = getattr(self, helperName, None)
        if _debug: MultiDeviceApplication._debug("    - helperFn: %r", helperFn)
        ...
        try:
            ...
            if _debug: MultiDeviceApplication._debug("    - execution error: %r", err)
            ...
            MultiDeviceApplication._exception("exception: %r", err)
    ...
    def do_WhoIsRequest(self, apdu):
        """Respond to a Who-Is request."""
        if _debug:
            MultiDeviceApplication._debug("do_WhoIsRequest %r", apdu)

        # may be a restriction
        for localDeviceID in self.localDevices:
            localDevice = self.localDevices[localDeviceID]
            if ((apdu.deviceInstanceRangeLowLimit is not None) and (apdu.deviceInstanceRangeHighLimit is not None)):
                if (self.localDevice.objectIdentifier[1] < apdu.deviceInstanceRangeLowLimit):
                    continue
                if (self.localDevice.objectIdentifier[1] > apdu.deviceInstanceRangeHighLimit):
                	continue
            ...
            iAm = IAmRequest()
            iAm.pduDestination = apdu.pduSource
            iAm.iAmDeviceIdentifier = localDevice.objectIdentifier
            iAm.maxAPDULengthAccepted = localDevice.maxApduLengthAccepted
            iAm.segmentationSupported = localDevice.segmentationSupported
            iAm.vendorID = localDevice.vendorIdentifier
            if _debug:
                MultiDeviceApplication._debug("    - iAm: %r", iAm)
        	# away it goes
            self.request(iAm)
    ...
    def do_WritePropertyRequest(self, apdu):
        """Change the value of some property of one of our objects."""
        if _debug: MultiDeviceApplication._debug("do_WritePropertyRequest %r", apdu)
    ...
    def do_AtomicReadFileRequest(self, apdu):
        """Return one of our records."""
        if _debug: MultiDeviceApplication._debug("do_AtomicReadFileRequest %r", apdu)

        if (apdu.fileIdentifier[0] != 'file'):
            resp = Error(errorClass='services', errorCode='inconsistentObjectType', context=apdu)
            if _debug: MultiDeviceApplication._debug("    - error resp: %r", resp)
            self.response(resp)
            return
        ...
        if not obj:
            ...
            ### verify start is valid - double check this (empty files?)
                ...
                if _debug: MultiDeviceApplication._debug("    - record_data: %r", record_data)
    ...
    def do_AtomicWriteFileRequest(self, apdu):
        """Return one of our records."""
        if _debug: MultiDeviceApplication._debug("do_AtomicWriteFileRequest %r", apdu)
        ...
        if not obj:
            ...
            if _debug: MultiDeviceApplication._debug("    - start_record: %r", start_record)
            ...
            if _debug: MultiDeviceApplication._debug("    - start_position: %r", start_position)
...
#
#   BIPMultiDeviceSimpleApplication
#

class BIPMultiDeviceSimpleApplication(MultiDeviceApplication, Logging):

    def __init__(self, localDevices, localAddress, aseID=None):
        if _debug:
            BIPMultiDeviceSimpleApplication._debug("__init__ %r %r aseID=%r", localDevices, localAddress, aseID)
        MultiDeviceApplication.__init__(self, localDevices, localAddress, aseID)
        ...
        # pass the device object to the state machine access point so it
        # can know if it should support segmentation
        print type(localDevices)
        devs = localDevices.values()
        print devs
        self.smap = StateMachineAccessPoint(devs[0])
